...
int hnj_hyphen_hyphenate_js (const char *fdata, size_t flen,
        const char *word, int word_size, char * hyphens,
        char *hyphword, char *** rep, int ** pos, int ** cut){
	if(!currentJsDict){
		printf ("Creating dict \n");
		currentJsDict = hnj_hyphen_load_data(fdata,flen);
	}
	
	printf ("Calling \n");
	
	  hnj_hyphen_hyph_(currentJsDict, word, word_size, hyphens, rep, pos, cut,
    currentJsDict->clhmin, currentJsDict->crhmin, 1, 1);
  hnj_hyphen_lhmin(currentJsDict->utf8, word, word_size,
    hyphens, rep, pos, cut, (currentJsDict->lhmin > 0 ? currentJsDict->lhmin : 2));
  hnj_hyphen_rhmin(currentJsDict->utf8, word, word_size,
    hyphens, rep, pos, cut, (currentJsDict->rhmin > 0 ? currentJsDict->rhmin : 2));

  /* nohyphen */
  if (currentJsDict->nohyphen) {
    char * nh = currentJsDict->nohyphen;
    int nhi;
    for (nhi = 0; nhi <= currentJsDict->nohyphenl; nhi++) {
        char * nhy = (char *) strstr(word, nh);
        while (nhy) {
            hyphens[nhy - word + strlen(nh) - 1] = '0';
            if (nhy - word  - 1 >= 0) hyphens[nhy - word - 1] = '0';
  ...
  if (hyphword){ 
	  hnj_hyphen_hyphword(word, word_size, hyphens, hyphword, rep, pos, cut);
	  printf ("HYPHENATED: %s\n", hyphword);
  }
  if (currentJsDict->utf8) return hnj_hyphen_norm(word, word_size, hyphens, rep, pos, cut);
